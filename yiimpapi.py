...
#zergpool_workers = zergpool_data['INN']['workers']
#zergpool_hashrate = float(zergpool_data['INN']['hashrate']) / 1000000000

#bsod
#bsod_url = requests.get('http://api.bsod.pw/api/currencies')
#bsod_data = json.loads(bsod_url.text)
#bsod_workers = bsod_data['INN']['workers']
#bsod_hashrate = float(bsod_data['INN']['hashrate']) / 1000000000

#cafe_url = requests.get('http://mining.cafe/api/currencies')
#cafe_data = json.loads(cafe_url.text)
#cafe_workers = cafe_data['INN']['workers']
#cafe_hashrate = float(cafe_data['INN']['hashrate']) / 1000000000
#cafe_block = float(cafe_data['INN']['height'])
